...
`claude_pool` is a small CLI (`claudes.py`) for running multiple Codex
...
- **Account model**: `accounts.json` is a list of `{name, config_path}`
  entries. Each account's Codex state (credentials, sessions,
  settings) lives under `profiles/<name>/`. Switching accounts means
  invoking `Codex` with `CLAUDE_CONFIG_DIR` set to that account's
...
- **`accounts.json` and `profiles/` are gitignored** and hold live,
  per-machine state (account assignment metadata, real Codex credentials
